    ...
    <div className="h-screen flex flex-col pt-4 overflow-hidden">
      ...
      <div className="px-8 pb-4 border-b border-white/5 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-400">Mobile App Redesign Sprint</p>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="flex -space-x-2 mr-4">
            {['1','2','3'].map(u => (
              <img key={u} src={`https://i.pravatar.cc/150?u=${u}`} alt="User" className="w-8 h-8 rounded-full border-2 border-slate-900" />
            ))}
            <div className="w-8 h-8 rounded-full border-2 border-slate-900 bg-slate-800 flex items-center justify-center text-xs text-slate-300">
              +2
            </div>
          </div>
          ...
          <div className="relative hidden md:block">
            ...
            <input
              ...
              className="bg-white/5 border border-white/10 rounded-lg pl-9 pr-3 py-1.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 w-48"
            />
            {searchQuery && (
          ...
          <div className="relative">
            ...
            {showFilters && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-white/10 rounded-xl shadow-xl z-50 p-2">
                <p className="text-xs font-semibold text-slate-400 uppercase px-3 py-2">Priority</p>
                {['all', 'urgent', 'high', 'medium', 'low'].map(p => (
                  <button
                    key={p}
                    onClick={() => { setFilterPriority(p); setShowFilters(false); }}
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm capitalize transition-colors ${filterPriority === p ? 'bg-indigo-600/20 text-indigo-400' : 'text-slate-300 hover:bg-white/5'}`}
                  >
