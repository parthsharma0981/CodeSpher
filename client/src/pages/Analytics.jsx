    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto text-white">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex items-center space-x-3">
          <select className="bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:outline-none focus:border-indigo-500 appearance-none">
            <option>This Week</option>
            <option>This Month</option>
            <option>This Quarter</option>
          </select>
          <button className="flex items-center space-x-2 bg-white/10 hover:bg-white/20 border border-white/10 px-4 py-2 rounded-lg text-sm font-medium transition-colors backdrop-blur-md">
            <Download size={16} />
            <span>Export</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          ...
          { label: 'Productivity Score', value: '87%', trend: '+5%', icon: TrendingUp, color: 'text-indigo-400', bg: 'bg-indigo-400/10' },
          { label: 'Team Members', value: '24', trend: '+3', icon: Users, color: 'text-cyan-400', bg: 'bg-cyan-400/10' },
          ...
          return (
            <motion.div
              ...
              key={i}
              className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-xl hover:bg-white/[0.07] transition-colors"
            >
              ...
              <div key={i} className="w-full flex flex-col items-center group">
                <div className="w-full relative h-48 bg-white/5 rounded-t-md overflow-hidden flex items-end">
                  <motion.div
                    ...
                    className="w-full bg-gradient-to-t from-indigo-600 to-cyan-400 rounded-t-md group-hover:opacity-80 transition-opacity relative"
                  >
        ...
        >
          ...
          <div className="space-y-6">
            {[
              ...
              { name: 'Mobile App API', progress: 60, color: 'from-indigo-500 to-purple-500' },
              ...
              {[
                ...
                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${i === 0 ? 'bg-amber-500/20 text-amber-400' : i === 1 ? 'bg-gray-400/20 text-gray-300' : 'bg-amber-700/20 text-amber-600'}`}>
                      #{i + 1}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center text-indigo-400 font-bold">{member.score}</td>
                </tr>
              ))}
            </tbody>
          </table>
